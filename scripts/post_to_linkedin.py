...
Optional env vars:
  ...
  LINKEDIN_DELETE_POST_URN  Delete this share or UGC post before posting
...
def delete_linkedin_post(post_urn, access_token):
    """Delete one LinkedIn share or UGC post by URN."""
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            if resp.status != 204:
                raise RuntimeError(f"Unexpected LinkedIn delete status: {resp.status}")
    except urllib.error.HTTPError as e:
        body = e.read().decode("utf-8", errors="replace")
        print(f"LinkedIn delete error {e.code}: {body}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    delete_post_urn = os.environ.get("LINKEDIN_DELETE_POST_URN", "").strip()
    if delete_post_urn:
        print(f"Deleting LinkedIn post: {delete_post_urn}")
        delete_linkedin_post(delete_post_urn, access_token)
        print(f"Deleted LinkedIn post: {delete_post_urn}")
    ...
    unknown_langs = [lang for lang in seq if lang not in LANGUAGES]
    if unknown_langs:
        print(
            f"Error: unsupported LinkedIn languages: {', '.join(unknown_langs)}.",
            file=sys.stderr,
        )
        sys.exit(1)
    missing_pages = [
        lang for lang in seq
        if _read_local_html(_localized_url(entry["url"], lang)) is None
    ]
    if missing_pages:
        print(
            "Error: refusing to post languages without local article pages: "
